...
struct input_arr read_inputfile(const char* filename) {
    ...
    for(int i = 0; i < ret.num_elems; i++) {
        ...
        // Day 13:
        if(i == 0) ret.departure_time = atoll(token_list[i]);
        else ret.vec.push_back(parse_elem(token_list[i]));
        //ret.arr[i] = parse_elem(token_list[i]);
        //ret.vec.push_back(parse_elem(token_list[i])); // Parse a single element at a time
    }
    
    // Day 13 has special input format

    // Line 1: departure time
    ret.departure_time = atoll(token_list[0]);

    // Line 2: comma delimited list of bus_ids 
    // TODO: x's?
